...
except Exception:  # pragma: no cover - optional dependency errors are handled gracefully
    UTCPClient = None
...
class UTCPTool(CoalaTool):
    ...
    async def _call_via_client(self, arguments: Dict[str, Any]):
        if not self._client:
            return None
        call_fn = getattr(self._client, "call_tool", None) or getattr(self._client, "call", None)
        if not call_fn:
            return None
    ...
    async def _call_via_http(self, arguments: Dict[str, Any]):
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"{self.endpoint}/call", json=payload, headers=headers)
