...
from coala.tools.coala_tool import CoalaTool
...
class MCPServerRegistration:
    ...
    async def session(self):
        if self.transport == "http":
            ...
            async with streamable_http_client(self.server_url) as (read_stream, write_stream, _sid):
...
class ProceduralMemory:
    ...
    def register_mcp_server(
        ...
        if not server_url and not command:
            raise ValueError("MCP server registration requires a server_url or a command to run.")
    ...
    def _build_tool_from_provider(self, tool_name: str, provider: str) -> Optional[CoalaTool]:
        if provider == "manual":
        ...
        required = list(input_schema.get("required", [])) if isinstance(input_schema, dict) else []
        return MCPRegisteredTool(
            tool_name, getattr(tool_def, "description", "") or "", server, required_fields=required
        )
    ...
    async def sync_mcp_tools(self):
        ...
        for server in self.mcp_servers.values():
            ...
            for tool_def in getattr(tools_result, "tools", []) or []:
                ...
                required = (
                    list(input_schema.get("required", [])) if isinstance(input_schema, dict) else []
                )
            ...
            for tool_name in removed_tools:
                ...
                if not providers:
                    ...
                    next_owner = "manual" if "manual" in providers else next(iter(providers))
